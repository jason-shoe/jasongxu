...
import './AthleteBrandingComparison.css';
...
import ScatterPlot from "./graphs/1ScatterPlot.js"
import NBAdata from './graphs//1NBAScatterPlot.csv';
import NFLdata from './graphs//1NFLScatterPlot.csv';
import SalaryLorenz from './graphs/1SalaryLorenz.png';
import FollowerLorenz from './graphs/1FollowerLorenz.png';
import YearlyGrowth from './graphs/1YearlyGrowth.png';
...
class AthleteBrandingComparison extends Component {
  ...
  changeColor(index){
    if(this.state.color[index] == 'black'){
      this.state.color[index] = '#EA5455';
    }else{
      this.state.color[index] = 'black';
    }
    this.forceUpdate();
  }
    ...
    <body class="container">
      ...
      <MathJax.Provider>
        ...
        <div class="article-content">
          ...
          <p>How I quantified how big a player's brand is was by their Instagram follower count. This is how most
             athletes reach their followers, and thus I used this as my primary metric.</p>
          <h2>Data Collection</h2>
          <p>Most of the webscraping was done using Beautiful Soup on ESPN, basketball-reference, and football-reference.
             Two interesting aspects of the webscraping process was retrieving Instagram handles and follower counts, which was collected on
             March 19, 2020.</p>
          <p>To get the player Instagram handles, I googled the players name along with "Instagram NBA" or "Instagram NFL" and retrieved
             the first result.</p>
          ...
          <div id = "transition" class="bio_resume">
            <a href={NBAdata} download={NBAdata}><FaFilePdf onMouseEnter = {() => this.changeColor(0)} onMouseLeave = {() => this.changeColor(0)} color={this.state.color[0]} size="2em"/></a>
            <p class="resume_label">NBA Data</p>
          </div>
          <div id = "transition" class="bio_resume">
            <a href={NFLdata} download={NFLdata}><FaFilePdf onMouseEnter = {() => this.changeColor(1)} onMouseLeave = {() => this.changeColor(1)} color={this.state.color[1]} size="2em"/></a>
            <p class="resume_label">NFL Data</p>
          </div>
          <h2>Data Visualization</h2>
          <p>I first just plotted the collected data on scatter plots to see if any trends
             are immedietely apparent.</p>
          <div class="scatter-plot-graphs">
          ...
          <div class="lorenz-description-div">
            <div class="lorenz-div">
              <p id="lorenz-header">Salary Distribution</p>
              <img class="lorenz-curve" src = {SalaryLorenz} />
              <p id="lorenz-header">Follower Distribution</p>
              <img class="lorenz-curve" src = {FollowerLorenz} />
            </div>
          ...
          <img class="yearly-growth-image"src={YearlyGrowth} />
          <h2>Findings</h2>
          ...
          <p>Despite longer careers, larger salaries, and a smaller pool of athletes, the average NBA player
             ...
             false. Even though NBA players entire the league with larger followings, the NFL has allowed for comprable, if not 
